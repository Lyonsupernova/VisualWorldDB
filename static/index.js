...
async function fetchData() {
  objectNumberMapping.clear()
   try {
    const videoDataResponse = await fetch('/fetch-video/dataset_1/video_00000.mp4')
    const videoDataResponseJson = await videoDataResponse.json()
    console.log("video data response: ", videoDataResponseJson)
    const boundingBoxData = []


    const videoName = videoDataResponseJson.video_id;
    const videoId = 0
    for(let i = 0; i < videoDataResponseJson.frame_id.length; i++) {
      ...
      const boundingBoxDataResponse = await fetch(`/box/dataset_1/${videoId}/${frameNum}`)
